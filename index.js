...
function sentNewMovieToTel(movieThumbnail, id) {
  try {
    axios
      .get(
        `https://api.telegram.org/bot6465806242:AAH5As3iEipDMow9d8IM8bmGXya3udjEgYM/sendPhoto?chat_id=-1002499362787&photo=${movieThumbnail}&caption=*New Movie Added*:- %0A %0A[Download Link](https://vacancies-v1f4.onrender.com/movies?id=${id}) %0A %0ASearch Any Movie From Here - https://vacancies-v1f4.onrender.com/movies/search %0A %0A*Want to create your own channel like this contact @sunilkumar_utk*&parse_mode=markdown`
      )
      .then((res) => {
    ...
    axios.get(dblLink).then((res) => {
      ...
      fs.readFile("./finalfilesdlink.json", "utf-8", (err, data) => {
        if (err) {
          ...
          //upload to cloudinary and then send to tel
          const cloudinary = require("cloudinary").v2;
          cloudinary.config({
            cloud_name: "dwxkccwul",
            api_key: 911613729936569,
            api_secret: "ltr7yEKogFvjhQQ_bublkKcP_dI",
          });

          const image = allEpisodesFilesDlink[0].movieThumbnail;

          cloudinary.uploader.upload(image).then((res) => {
            console.log(res.secure_url);
            sentNewMovieToTel(res.secure_url, allEpisodesFilesDlink[0].id);
          });
    ...
    .then((res) => {
      ...
      $("body").each((index, element) => {
        let name = $(element)
          ...
          .find("div:nth-child(15)>table>tbody>tr>td>a:nth-child(1)")
          .attr("href");
  ...
  if (req.query.id) {
    fs.readFile("./finalfilesdlink.json", "utf-8", (err, data) => {
      if (err) {
        ...
        if (fsdata) {
          const innerHtml = axios
            .get(fsdata.filesDlLink)
            .then((res) => {
...
app.get("/movies/search", (req, res) => {
  if (req.query.query) {
    try {
      ...
      currentUrl.then((url) => {
        axios
          .get(url + "/site-1.html?to-search=" + req.query.query)
          .then((res) => {
            const $ = cheerio.load(res.data);
            let searchHtmlData = $("body")
              .find("div.A2>a>img")
              .parent()
              .wrapInner("<div class='loadingAnimationPoster'></div>")
              .toString();

            // searchHtmlData = $(searchHtmlData).find("a").attr("href");

            // console.log($("body").find("div.A2").length)
            // fs.writeFile("./searchFilmy.html", searchHtmlData, (err) => {
            //   console.log(err);
            // });
            return searchHtmlData;
          })
          .then((searchHtmlData) => {
            if (req.query.axios) {
              res.json(searchHtmlData);
            } else {
              if(searchHtmlData.length < 2){
                searchHtmlData = "<h1>No Result Found<h1>"
              }
              res.render("searchMoviepage", {
                searchedValue: req.query.query,
                searchHtmlData,
              });
            }
          })
          .catch((err) => {
            res.send("err Ho gaya");
          });
      });
    } catch (error) {
      console.log(error);
...
app.get("/page-download/:id/:id2", (req, response) => {
  getDownloadLinkFromMoviePageUrl(
    "https://filmyfly.golf/page-download/" +
      req.params.id +
      "/" +
      req.params.id2
  );
      ...
      .then((res) => {
        ...
        axios
          ...
          .then((res) => {
            ...
            // let webSeriesUpperText = $(linkMakeDblLink).text();
            // console.log(webSeriesUpperText);
            linkMakeDblLink = $(linkMakeDblLink)
              .find("center")
              .first()
              .toString();
            ...
            fs.readFile("./finalfilesdlink.json", "utf-8", (err, data) => {
              if (err) {
                ...
                const searchresultId = allEpisodesFilesDlink[0].id;
                ...
                return response.redirect("/movies?id=" + searchresultId);
              }
            });
            // finallOneClickDlink = [];
            // allEpisodesFilesDlink.forEach((link) => {
          ...
          .catch((err) => {
            response.send("err ho gaya bhai Plz Go back");
          });
      })
      .catch((err) => {
        response.send("err ho gaya bhai");
      });
  }
});

app.get("/music", (req, res) => {
  fs.readFile("./songDetails.json", "utf-8", (err, data) => {
    if (err) {
      console.log(err);
    } else {
      const fsdata = JSON.parse(data);
      res.render("musicHomepage", {
        route: "/music",
        fsdata,
      });
    }
